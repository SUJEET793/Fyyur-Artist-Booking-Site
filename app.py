...
def venues():
    ...
    for area in venue_areas:
        venues = db.session.query(Venue.id, Venue.name,
                                Venue.upcoming_shows_count).filter(Venue.city == area[0],
...
def create_venue_submission():
    ...
    try:
        print("try"+new_venue.seeking_description)
        db.session.add(new_venue)
        print("before commit")
        db.session.commit()
        print("after coomit")

        print("after session"+new_venue.seeking_description)
...
def create_show_submission():
    ...
    new_show.start_time = datetime(
        DTList[0], DTList[1], DTList[2], DTList[3], DTList[4], DTList[5])
    now = datetime.now()
